...
import { HelpCircle, Upload, MessageCircle, Send, Link as LinkIcon, Mail, Pencil, Trash2, Loader2 } from 'lucide-react';
import { toast } from '../hooks/use-toast';
import { useAuth } from '../contexts/AuthContext';
import { pricingAPI, cartAPI, orderAPI } from '../services/api';

const PrintingWithBackend = () => {
  ...
  useEffect(() => {
    const loadPricing = async () => {
      try {
        ...
        toast({
          title: 'خطا',
          description: 'خطا در بارگذاری اطلاعات قیمت',
          variant: 'destructive'
        });
      }
    };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      toast({
        title: 'خطا',
        description: error.response?.data?.detail || 'خطا در افزودن به سبد خرید',
        variant: 'destructive'
      });
    }
  ...
  const handleRemoveFromCart = async (itemId) => {
    try {
      ...
      toast({
        title: 'خطا',
        description: 'خطا در حذف از سبد خرید',
        variant: 'destructive'
      });
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      toast({
        title: 'خطا',
        description: error.response?.data?.detail || 'خطا در ثبت سفارش',
        variant: 'destructive'
      });
    }
  };
              ...
              <CardContent className=\"p-6 space-y-4\">
                ...
                <div className=\"space-y-2 pt-4\">
                  <Button
                    ...
                    disabled={loadingCart || calculating}
                  >
                    {loadingCart ? (
                      <>
...
export default PrintingWithBackend;
